...
void handle_command(uint8_t *buffer, uint8_t sock) {
    ...
    if (strcmp((char *)buffer, "On1") == 0) {
        ...
        status_msg[0] = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET) ? 'a' : 'x';
        status_msg[1] = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_1) == GPIO_PIN_SET) ? 'b' : 'y';
        status_msg[2] = (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_2) == GPIO_PIN_SET) ? 'c' : 'z';
